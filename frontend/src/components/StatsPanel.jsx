...
function StatsPanel() {
  ...
  return (
    <div className="glass-card flex flex-col gap-6 rounded-2xl p-7">
      <div className="mb-2">
        <h2 className="text-2xl font-bold text-[#F5E6D3]">Statistics</h2>
      </div>
        ...
        {["total_items", "unique_locations", "items_last_7_days"].map((key) => {
          ...
          return (
            <div key={key} className="rounded-xl border-2 border-[#F5E6D3]/20 bg-[#F5E6D3]/5 p-5 transition-all duration-300">
              ...
              <dd className="mt-3 text-4xl font-bold text-[#F5E6D3]">{stats[key]}</dd>
            </div>
          );
        })}
      </dl>

      <div className="text-base text-[#F5E6D3]/90 bg-[#F5E6D3]/5 rounded-xl p-5 border-2 border-[#F5E6D3]/20">
        <p>
          <span className="font-bold text-[#F5E6D3]">Last Upload:</span>{" "}
          {formatTimestamp(stats.last_uploaded_at)}
        </p>

        {stats.top_locations.length > 0 && (
          <p className="mt-2">
            <span className="font-bold text-[#F5E6D3]">Hotspots:</span>{" "}
            {stats.top_locations.map((loc, idx) => (
              <span key={loc.location}>
                {idx > 0 && ", "}
                {loc.location} <span className="text-[#FF8C42]">({loc.count})</span>
              </span>
            ))}
          </p>
        )}
