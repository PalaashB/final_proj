...
class SemanticEngine:
    ...
    def _encode_image(self, image: Image.Image) -> torch.Tensor:
        with torch.no_grad():
            image_input_full = self.preprocess(image).unsqueeze(0).to(self.device)
            emb_full = self.model.encode_image(image_input_full)
            ...
            image_input_crop = self.preprocess(cropped_image).unsqueeze(0).to(self.device)
            emb_crop = self.model.encode_image(image_input_crop)

            combined_embedding = (emb_full + emb_crop) / 2.0
            combined_embedding = combined_embedding / combined_embedding.norm(dim=-1, keepdim=True)
            
        return combined_embedding.cpu()[0]

    def _encode_text(self, text: str) -> torch.Tensor:
        templates = [
            "a photo of a {}",
            "a close-up photo of a {}",
            "a lost {}, sitting on a surface",
            "an image of {}"
        ]
    ...
    def add_item(self, *, image_bytes: bytes, location: str, finder_contact: str) -> Dict[str, Any]:
        if not image_bytes:
        ...
        with self.lock:
            existing = self.conn.execute("SELECT id FROM items WHERE image_hash = ?", (image_hash,)).fetchone()
            if existing:
        ...
        image_name = f"{timestamp.replace(':', '').replace('-', '')}_item.jpg"
        image_path = self.upload_dir / image_name
        pil_image.save(image_path, format="JPEG", quality=92)

        payload = (
            location or "", finder_contact or "",
            image_name, sqlite3.Binary(vector_bytes), vector_dim, image_hash,
            timestamp, timestamp
        )
    ...
    def search_items(self, *, query: str, threshold: float, top_k: int, location: Optional[str] = None) -> List[Dict[str, Any]]:
        if not query.strip():
            return []
    ...
    def get_stats(self) -> Dict[str, Any]:
        with self.lock:
            ...
            unique_locations = self.conn.execute("SELECT COUNT(DISTINCT location) FROM items WHERE location <> ''").fetchone()[0]
            
            last_uploaded = self.conn.execute("SELECT created_at FROM items ORDER BY created_at DESC LIMIT 1").fetchone()
            last_uploaded_at = last_uploaded[0] if last_uploaded else None

            items_last_7_days = self.conn.execute("SELECT COUNT(*) FROM items WHERE datetime(created_at) >= datetime('now', '-7 days')").fetchone()[0]
            
            top_locations_rows = self.conn.execute(
                "SELECT location, COUNT(*) as cnt FROM items WHERE location <> '' GROUP BY location ORDER BY cnt DESC LIMIT 5"
            ).fetchall()
